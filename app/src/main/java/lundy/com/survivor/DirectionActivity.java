...
public class DirectionActivity extends FragmentActivity implements OnMapReadyCallback, RoutingListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        latStart = getIntent().getDoubleExtra(RouteDistance.KEY_LAT_START, 0);
        lngStart = getIntent().getDoubleExtra(RouteDistance.KEY_LNG_START, 0);
        latEnd = getIntent().getDoubleExtra(RouteDistance.KEY_LAT_END, 0);
        lngEnd = getIntent().getDoubleExtra(RouteDistance.KEY_LNG_END, 0);
        ...
        Routing routing = new Routing.Builder()
                ...
                .key("AIzaSyA3Md_eUIbKFKUtkihc7rD2vWcEmF7u-EI")
                .waypoints(start, end)
                .withListener(this)
                .build();
        routing.execute();
    ...
    public void onRoutingSuccess(ArrayList<Route> routes, int i) {
        ...
        for (Route data : routes) {
            ...
            Toast.makeText(getApplicationContext(),data.getDistanceText(),Toast.LENGTH_SHORT).show();
